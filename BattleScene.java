...
public class BattleScene {
    ...
    public void fight(FantasyCharacter hero, FantasyCharacter monster, Game.FightCallback fightCallback) {
    ...
    private Boolean makeHit(FantasyCharacter defender, FantasyCharacter attacker, Game.FightCallback fightCallback) {
        //Получаем силу удара
        ...
        if (doubleHit != 0) {
            System.out.printf("%s Нанес критический удар в %d единиц!%n", attacker.getName(), hit);
            System.out.printf("Враг повержен! Вы получаете %d опыт и %d золота в удвоенном размере!%n", defender.getXp(), defender.getGold());
            attacker.setXp((attacker.getXp() + defender.getXp()) * 2);
            attacker.setGold((attacker.getGold() + defender.getGold()) * 2);
